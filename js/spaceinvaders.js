...
function SchoolGen() {
  var schoolsData = getSchools();
  var randNumMin = 0;
  var randNumMax = schoolsData.length-1;
  this.rand = function () {
    return schoolsData[(Math.floor(Math.random() * (randNumMax - randNumMin + 1)) + randNumMin)];
  }
}
schoolsGen = new SchoolGen();

function KillFeed() {

  var schools = [];

  this.add = function(school,x,y) {
    schools.push({
      name: school,
      x     : x,
      y     : y,
...
WelcomeState.prototype.draw = function(game, dt, ctx) {
    ...
    ctx.font="30px Arial";
    ctx.fillStyle = '#CA4443';
    ctx.textBaseline="center";
    ctx.textAlign="center";
    ctx.fillText("Help Grapefruit spread to all universities in the galaxy!", game.width / 2, game.height/2 - 40);
    ctx.font="16px Arial";
    ...
    for(var rank = 0; rank < ranks; rank++){
        for(var file = 0; file < files; file++) {
            invaders.push(new Invader(
                ...
                rank, file, 'Invader',schoolsGen.rand()));
        }
    }
    ...
    for(i=0; i<this.invaders.length; i++) {
        ...
        for(var j=0; j<this.rockets.length; j++){
            ...
            if(rocket.x >= (invader.x - invader.width/2) && rocket.x <= (invader.x + invader.width/2) &&
                ...
                this.killFeed.add(invader.schoolName,rocket.x,rocket.y);
...
PlayState.prototype.draw = function(game, dt, ctx) {
    ...
    for(var i=0; i<this.rockets.length; i++) {
        ...
        ctx.drawImage(grapefruit,rocket.x,rocket.y - 2, 15, 15);
    }
    ...
    ctx.fillStyle = '#ffffff';
    ctx.textAlign = "left";
    var dead = this.killFeed.getCurrent();
    var offsety = 45;
    ctx.fillText("Grapefruit Takeover Feed",15,20);
    ctx.font="200 14px Arial";
    for(var i=0; i < dead.length; i++) {
      ctx.fillText(dead[i],15,offsety);
      offsety+=18;
    }
...
function Invader(x, y, rank, file, type, schoolName) {
    this.x = x;
    this.y = y;
    this.rank = rank;
    this.file = file;
